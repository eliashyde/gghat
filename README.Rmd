...
in its repertoire. To this end, `gghat` contains `geom_hat()` as well as a compatible position 
function to be used with `geom_text()`. You can install the development version of `gghat` from [GitHub](https://github.com/eliashyde/gghat). Obviously, `gghat` also requires that `ggplot` is installed. 
